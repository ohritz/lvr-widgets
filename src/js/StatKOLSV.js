(function() {
    var s,
         ...
         */
            getListOfWeeks: function(nWeeksFromToday) {
                var currentDate = new Date(), i, weeks = [];
                for (i = 0; i < nWeeksFromToday; i++) {
                    weeks.push(
                        Ext.Date.format(
                            Ext.Date.subtract(currentDate, Ext.Date.DAY, 7 * i),
                            'Y-W'
                        )
                    );
                }
         ...
         */
            sumWeeks: function(nWeeksFromToday, weekStatistics) {
                var weeks = this.getListOfWeeks(nWeeksFromToday), sum = 0;
            ...
            _calculateSelectedIndicator: function(id) {
                ...
                currIndicator.value = sum > 0 && selectedIndicator
                    ? 100 * selectedIndicator / sum
                    : 0;
            },
            ...
            getData: function(opts) {
                ...
                opts._retData = opts._retData || {sums: {}};
                opts._retData.sums[clinicAll] = 0;
                opts._retData[clinicAll] = {};
                Ext.Ajax.request({
                    //TODO: Fix relative path
                    method: 'get',
                    url: '/api/aggregate/LVR/Inpatient/' +
                        (opts.singleUnit ? 'Unit/' : 'Total/') +
                        opts.url +
                        '/week(InpDate)',
                    success: function(response) {
                        var rData = Ext.decode(response.responseText);

                        Ext.Object.each(rData.data, function(
                            value,
                            weekValues
                        ) {
                            opts._retData.sums[clinicAll] += opts._retData[clinicAll][value ===
                                'null'
                                ? 'Okänt'
                                : value] = OVWidget.sumWeeks(65, weekValues);
                        });
                        ...
                        if (!opts.singleUnit) {
                            ...
                            OVWidget._dataCollectedCallback(
                                opts._retData,
                                opts.id,
                                opts.callback
                            );
            ...
            getAllData: function(callbackFn) {
                ...
                Ext.Object.each(s.indicators, function(id, indicator) {
                    s.nCallbacks += typeof indicator.sums === 'undefined'
                        ? 1
                        : 0; //check if indicator data is already loaded
                });
                Ext.Object.each(s.indicators, function(id, indicator) {
    ...
    OVWidget.getAllData(function(data) {
        var chart = Ext.create('Ext.chart.Chart', {
            store: Ext.create('Ext.data.Store', {fields: ['unit', 'value']}),
            // theme: 'LVRTheme',
            hidden: true,
            animate: true,
            shadow: false,
        ...
        Ext.create('Ext.container.Container', {
            ...
            items: Repository.Local.Methods.getSmallGaugesInits(
                data,
                chart,
                true
            )
